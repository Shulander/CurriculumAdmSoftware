...
<?php include ("cabecalho.php");
	...
	//testa se a variavel idExpAcad existe
	if(isset($_REQUEST['idExpAcad'])) {
		$idExpAcad = $_REQUEST['idExpAcad'];	
	} else {
		$idExpAcad = "";
	}
?>
<!-- Sub-titulo -->
<h3>Editar Formação Acadêmica</h3>
...
function abreCursosUniversidade(selecionado,formId) {
	for(var i=0; i< $aUniversidades.length; i++) {
		if($aUniversidades[i] == selecionado){
			blocoAbre($('curso_'+formId+'_'+i)); 
			$('curso_'+formId).value=$('curso_'+formId+'_'+i).options[$('curso_'+formId+'_'+i).selectedIndex].value
		} else {			
			blocoFecha($('curso_'+formId+'_'+i)); 
		}
	}
}
//-->
...
if (!$conexaoBD->conecta()) {
	...
	if (isset($idLogin)) {
		...
		$resultado = $expAcademicas->busca();
		if (is_array($resultado)) {
			$numExpAcademicas = count($resultado);
			for ($i = 0; $i < $numExpAcademicas; $i++) {
				$idExpAcad = $resultado[$i];
				$expAcademica = new ExpAcademica ($resultado[$i], $idLogin, 0, "", "", "", "", "", "", "", $conexaoBD);
				$result = $expAcademica->buscaPorIdPessoa();
				if ($result == true) {
					$instituicao = $expAcademica->getInstituicao();
					$tipo = $expAcademica->getTipo();
					$curso = $expAcademica->getCurso();
					$turno = $expAcademica->getTurno();
					$semestre = $expAcademica->getSemestre();
					$dataIngresso = $expAcademica->converteDataIngresso();
					$dataConclusao = $expAcademica->converteDataConclusao();
					echo '<ul>';
					echo '<li>'.$curso.' - '.$tipo.' | <a href="javascript: void(0);" onclick="blocoExibe(\'expAcademica\', '.($i+1).', '.$numExpAcademicas.');">editar</a>';
					echo '<span id="expAcademica'.($i+1).'" style="display: none;">';
					//---FORM
					echo '<form action="dadosEducacionaisEditaBD.php" method="POST" name="dadosEducacionaisForm"
					 onsubmit="return verificaFormDadosEducacionais($(\'curso\'), $(\'tipo\'), $(\'instituicao\'), $(\'dataIngresso\'));">';
					echo '<table class="tabela">';
					//--------instituicao---------------
					echo '<input type="hidden" id="idExpAcad" name="idExpAcad" value='.$idExpAcad.' \>';
					echo '<tr><td>Instituição: <font class="erro">*</font></td>';
					echo '<td><select id="instituicao" name="instituicao" onchange="
					abreCursosUniversidade(this.options[this.selectedIndex].value, '.$i.'); 
					...
					//campo curso escondido
					echo '<input type="hidden" name="curso" id="curso_'.$i.'" value="'.$curso.'" />';
					//Cursos FADISMA
					echo '&nbsp;&nbsp;<select id="curso_'.$i.'_0" '.($instituicao == "FADISMA"?"":'style="display:none"').'  onchange="$(\'curso_'.$i.'\').value=this.options[this.selectedIndex].value">';
					echo '<option value="0"> -- Selecione o curso -- </option>';
					echo '<option value="Direito" '.($curso == "Direito"?'selected="selected"':"").'>Direito</option>';
					echo '</select>';	
					//Cursos FAMES
					echo '<select id="curso_'.$i.'_1" '.($instituicao == "FAMES"?"":'style="display:none"').' onchange="$(\'curso_'.$i.'\').value=this.options[this.selectedIndex].value">';
					echo '<option value="0"> -- Selecione o curso -- </option>';
					...
					echo '</select>';
					//Cursos FAPAS
					echo '<select id="curso_'.$i.'_2" '.($instituicao == "FAPAS"?"":'style="display:none"').' onchange="$(\'curso_'.$i.'\').value=this.options[this.selectedIndex].value">';
					echo '<option value="0"> -- Selecione o curso -- </option>';
					echo '<option value="Administração" '.($curso == "Administração"?'selected="selected"':"").'>Administração</option>';
					echo '<option value="Administração Hospitalar" '.($curso == "Administração Hospitalar"?'selected="selected"':"").'>Administração Hospitalar</option>';
					echo '<option value="Direito" '.($curso == "Direito"?'selected="selected"':"").'>Direito</option>';
					...
					echo '</select>';	
					//Cursos FASCLA
					echo '<select id="curso_'.$i.'_3" '.($instituicao == "FASCLA"?"":'style="display:none"').' onchange="$(\'curso_'.$i.'\').value=this.options[this.selectedIndex].value">';
					echo '<option value="0"> -- Selecione o curso -- </option>';
					echo '<option value="Administração" '.($curso == "Administração"?'selected="selected"':"").'>Administração</option>';
					...
					echo '</select>';
					//Cursos UFSM
					echo '<select id="curso_'.$i.'_4" '.($instituicao == "UFSM"?"":'style="display:none"').' onchange="$(\'curso_'.$i.'\').value=this.options[this.selectedIndex].value">';
					echo '<option value="0"> -- Selecione o curso -- </option>';
					echo '<option value="Administração" '.($curso == "Administração"?'selected="selected"':"").'>Administração</option>';
					...
					echo '<select id="curso_'.$i.'_5" '.($instituicao == "ULBRA Santa Maria"?"":'style="display:none"').' onchange="$(\'curso_'.$i.'\').value=this.options[this.selectedIndex].value">';
					echo '<option value="0"> -- Selecione o curso -- </option>';
					echo '<option value="Administração" '.($curso == "Administração"?'selected="selected"':"").'>Administração</option>';
					echo '<option value="Arquitetura e Urbanismo" '.($curso == "Arquitetura e Urbanismo"?'selected="selected"':"").'>Arquitetura e Urbanismo</option>';
					echo '<option value="Direito" '.($curso == "Direito"?'selected="selected"':"").'>Direito</option>';
					...
					echo '</select>';	
					//Cursos UNIFRA
					echo '<select id="curso_'.$i.'_6" '.($instituicao == "UNIFRA"?"":'style="display:none"').' onchange="$(\'curso_'.$i.'\').value=this.options[this.selectedIndex].value">';
					echo '<option value="0"> -- Selecione o curso -- </option>';
					echo '<option value="Administração" '.($curso == "Administração"?'selected="selected"':"").'>Administração</option>';
					echo '<option value="Arquitetura e Urbanismo" '.($curso == "Arquitetura e Urbanismo"?'selected="selected"':"").'>Arquitetura e Urbanismo</option>';
					...
					echo '<td><input type="text" value="'.$dataIngresso.'" readonly id="dataIngresso'.$i.'" name="dataIngresso" size="10" maxlength="10">
					<input type="button" value="Calendário" onClick=displayCalendar($(\'dataIngresso'.$i.'\'),"dd/mm/yyyy",this)>';
					echo '</td></tr>';
					echo '<tr></tr>';				
					//-------Data de conclusao-----------
