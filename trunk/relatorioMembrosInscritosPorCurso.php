<?php
	...
	$sql = "SELECT distinct curso FROM login,pessoa,expacademica WHERE expacademica.tipo='graduação' AND login.id=pessoa.idLogin AND pessoa.id=expacademica.idPessoa AND login.tipo='membro' ORDER BY curso ASC";
	...
	for ($i = 0; $i < $numCursos; $i++) {
		...
		$sqlNome = "SELECT distinct login.id,nome from login,pessoa,expacademica WHERE login.id=pessoa.idLogin AND pessoa.id=expacademica.idPessoa AND  expacademica.curso='".$cursos[$i]."' AND login.tipo='membro' ORDER BY nome ASC";
