<?php
	...
	echo "<h3>Setar pagamento de inscrito</h3>";
	if(!empty($aviso)) {
		if ($aviso == "sucesso") {
			echo '<ul class="sucesso"><li>Pagamento setado com sucesso!</li></ul>';		
		} else {
	...
	$sql = "SELECT login.id, pessoa.nome FROM pessoa,login WHERE login.id=pessoa.idLogin AND login.tipo<>'admin' AND login.pago=0 ORDER BY nome ASC";
	$resultado = mysql_query($sql, $conexaoBD->getLink());
	$numLinhas = mysql_num_rows ($resultado);
	echo '<form action="pagamentoInsereBD.php" method="POST">';
	if ($resultado != 0) {
		...
		echo "</select></center>";
		echo '<br/>';
		echo '<center><input type=Submit value="Salvar" /></form></center>';
	} else {
