<?php
	...
	echo "<h3>Relatório de Inscritos com Inscrição Paga</h3>";
	...
	$sql = "SELECT nome FROM pessoa,login WHERE login.id=pessoa.idLogin and login.pago=1 ORDER BY nome ASC";
	...
	if ($resultado != 0) {
		...
		$aviso = "Não há nenhum inscrito no sistema que já pagou sua inscrição!";
		header("Location:relatorios.php?aviso=".$aviso);
		exit();
	}
