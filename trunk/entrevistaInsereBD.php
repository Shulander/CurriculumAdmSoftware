<?php
	...
	if(is_null ($aviso)) {
		if (!$validador->isSelecionado($tipo)) {
			...
			if ($area != "Administraчуo" && $area != "Engenharias" && $area != "Saњde" && $area != "TI" && $area != "Letras" && $area != "Indefinida") {
				$aviso = "Para intercambista, щ necessсrio selecionar dentre as opчѕes: Administraчуo, Engenharias, Saњde, TI, Letras ou Indefinida!";	
			}
		} else { //entao eh membro
			if (!($area != "Administraчуo" && $area != "Engenharias" && $area != "Saњde" && $area != "TI" && $area != "Letras" )) {
				$aviso = "Para membro, nуo deve selecionar-se as opчѕes: Administraчуo, Engenharias, Saњde, TI, Letras!";	
			}			
		}
	}
