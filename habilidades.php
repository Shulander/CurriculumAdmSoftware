	...
	if (!$conexaoBD->conecta()) {
		...
		if (isset($idLogin)) {
			...
			if ($resultado == true) { //se pessoa foi cadastrada
				...
				$result = $pessoa->busca();
				if ($result == true) {
					$habilidades = $pessoa->buscaHabilidades();
					if ($habilidades == false) {
						...
						echo '<tr><td><input type="checkbox" id="recrutamento" name="recursosHumanos[]" value="Recrutamento e Alocação" '.(isset($recursosHumanos["Recrutamento e Alocação"])?'CHECKED="CHECKED"':"").'>Recrutamento e Alocação</td><td></td></tr>';
						echo '</table><br />';
						echo '<hr />';
