<?php
	...
	if(is_null ($aviso)) {
		if (!$validador->isSelecionado($turno)) {
			$aviso = "Й necessбrio selecionar uma opзгo do campo 'Turno'!";	
		} else if(!$validador->isLetra($turno)) {
			$aviso = "Campo 'Turno' invбlido!";
		}
	}
	/*---------Semestre------------*/
	if(is_null ($aviso)) {
	...
	/*-----------Editar expAcademica------------------*/
	$expAcademica = new ExpAcademica ($idExpAcad, $idLogin, $idPessoa, $curso, $tipo, $instituicao, $turno, $semestre, 
	$dataIngressoBD, $dataConclusaoBD, $conexaoBD);
	$aviso = $expAcademica->edita ();
	if ($aviso != sucesso) {
		if (!empty ($instituicaoOutra)) {
			$instituicao = "outra";
		}
